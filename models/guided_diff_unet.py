...
class UNetModel(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.dims = config.data.dims
        self.image_size = config.data.effective_image_size
        self.in_channels = config.model.input_channels
        self.model_channels = config.model.model_channels
        self.out_channels = config.model.output_channels
        ...
        self.use_scale_shift_norm = config.model.use_scale_shift_norm
        self.use_new_attention_order = config.model.use_new_attention_order
        self.resblock_updown = config.model.resblock_updown
...
def permute_channels(haar_image, forward=True):
        permuted_image = th.zeros_like(haar_image)
        if forward:
            for i in range(4):
                if i == 0:
                    k = 1
...
@utils.register_model(name='guided_diffusion_UNET_multi_speed_haar')
class UNetModel_multi_speed_haar(UNetModel):
  ...
  def forward(self, haar_x:dict, labels):
    ...
    if self.scale_max_haar_depth == 0: #'a case'
